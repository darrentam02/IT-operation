# Enterprise IT Operations, Staff Monitoring & Vendor Financial Management System - v2
...
### Authentication
- **2FA is REQUIRED** for all logins.
...
4. **IT_COLLEAGUE** - internormal staff; updates own status.
...
- **Data Collection (internal staff):** pulled from **Jira API** (ticket/environment details).
- **Data Collection (vendors):** submitted via **vendor API**.
- **Realtime refresh:** **1 minute**.
...
- **PDFs:** 3 documents (user guides + IT procedures manual). Supabase Storage triggers chunking (500-token chunks, 50-token overlap) -> pgvector embeddings (HNSW index).
- **Hybrid Retrieval:** Cosine Similarity (pgvector) + Full-Text Search (tsvector).

### DeepSeek API Execution
- Route top 5 context chunks to DeepSeek API (`deepseek-v4-pro` for complex policy analysis; `deepseek-v4-flash` for general Q&A).
...
- **Authentication:** dedicated API key per vendor (100 vendors, each own key).
...
## 11. Agent Guardrails / Gotchas (READ FIRST before coding)

The DDL and specs above are the target state. The following gaps MUST be resolved by the agent during implementation or the app will not function.

### A. RLS Policies Are Incomplete
RLS is enabled on `profiles`, `staff_statuses`, `procurement_records`, `payment_schedules`, `cost_allocations`, `vendors`, and `audit_logs` — but **policies are only defined for `audit_logs` and `vendors`**. With RLS enabled and no policy, **all reads/writes will be denied**. You MUST add policies for every remaining table, per role:
- `profiles`: `SELECT` when `id = auth.uid()`; admins/FINANCE_AUDITOR read global.
- `staff_statuses`: user reads/writes own row (`user_id = auth.uid()`); TEAM_LEAD reads their team; admins read all.
- `procurement_records` / `cost_allocations` / `payment_schedules`: role-based access — FINANCE_AUDITOR read-only global; approvers see their actionable records; creators manage their own.
- `vendors`: only a `SELECT` policy exists. Add `INSERT`/`UPDATE` restricted to SUPER_ADMIN + FINANCE (onboarding).

### B. Circular Foreign Keys on `teams` / `profiles`
`teams.team_lead_id` references `profiles(id)` and `profiles.team_id` references `teams(id)`. Circular FKs complicate inserts. Strategy:
- Create `teams` first with `team_lead_id` as `NULL`, create all `profiles`, then backfill `teams.team_lead_id`.
- Or drop the FK on `teams.team_lead_id` and enforce it in application logic. Pick one and be consistent in the seed script.

### C. `profiles` RLS Blocks Login Profile Load
Without a select policy on `profiles`, the auth post-login flow that fetches the current user's role cannot run. Ensure at minimum a self-read policy exists before wiring any `getProfile()` call, otherwise login appears broken.

### D. `on_leave` → Deputy Activation Is Not In the DB
The DDL stores `on_leave` but defines **no trigger/function** to auto-activate the `DEPUTY_HEAD_OF_IT`. Implement one of:
- A Postgres **trigger** that, when a SUPER_ADMIN's `on_leave` flips to `true`, sets the linked `deputy_for_user_id` role active.
- OR a Supabase **Edge Function / scheduled job** that checks leave status and toggles deputy rights.
All actions taken by the deputy must write `acted_as_deputy = true` in `audit_logs`.

### E. Staff Stale Flag (Heartbeat)
`staff_statuses.is_stale` has no automation. Implement a scheduled job (Edge Function cron or `pg_cron`) that sets `is_stale = true` when `updated_at` is older than 4 hours and the status is an active shift.

### F. 2FA Enforced
Supabase Auth does not enforce 2FA (TOTP) by default. Wire TOTP via Supabase Auth and enforce it at login for all roles; block login until verified.

### G. Grant Execution on `match_knowledge_base`
The `match_knowledge_base` function is `LANGUAGE plpgsql` but has **no `SECURITY DEFINER`** and no explicit `GRANT`. By default it may run as the calling (anon) role and only be executable by the table owner. Add `SECURITY DEFINER` + `GRANT EXECUTE` to the appropriate role, and set a sensible `search_path` to avoid hijacking.

### H. DeepSeek Key Server-Side Only
Never expose the DeepSeek API key or the vector generation in the client. All embedding + DeepSeek calls must run in a Supabase Edge Function. The client only sends the query; the server returns citations.

### I. SUM(Allocation) == 100% Enforcement
The `cost_allocations` table only validates a single row's percentage (0–100). The **cross-row sum == 100%** constraint must be enforced via a trigger or in application logic at save time — the CHECK constraint alone is insufficient.

### J. Variance Tolerance Constants
3-way match tolerances (0% price, ±2% tax/shipping) and the RAG threshold (0.78) should be stored as config/constants in one place and referenced — do not hard-code in multiple files.

### K. Circular Seed Order
Because of the FK graphs (teams ⇄ profiles, vendors → procurement → allocations/payments), the seed script MUST be ordered: teams → profiles → vendors → procurement_records → cost_allocations → payment_schedules → staff_statuses. Run DDL first, then seed, then verify one row reads back per table.
