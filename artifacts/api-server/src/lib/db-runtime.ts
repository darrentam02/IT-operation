...
// ---------------------------------------------------------------------
// Budget import — allocation-only upsert.
// paid_amount is never written here: it is owned by the payments trigger
// (markPaid). incurred_amount is only written when the row provides it, so a
// pure allocation refresh (idempotent re-import) leaves movements untouched.
// ---------------------------------------------------------------------
export type BudgetImportRowInput = {
  fiscalYear: number;
  category: string;
  allocated: number;
  incurred?: number;
...
export async function importBudgetRows(
  rows: BudgetImportRowInput[],
): Promise<{ imported: number; updated: number; years: number[] } | null> {
  const pool = await getPool();
  if (!pool) return null;
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    // Upsert arbiter: ensure the (fiscal_year, category) unique index exists.
    await client.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS budget_lines_fiscal_year_category_key
         ON public.budget_lines (fiscal_year, category)`,
    );
    let imported = 0;
    let updated = 0;
    const years = new Set<number>();
    for (const r of rows) {
      years.add(r.fiscalYear);
      const res = await client.query(
        `INSERT INTO public.budget_lines (fiscal_year, category, allocated_amount, incurred_amount)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (fiscal_year, category)
         DO UPDATE SET
           allocated_amount = EXCLUDED.allocated_amount,
           incurred_amount = CASE WHEN $5 THEN EXCLUDED.incurred_amount ELSE budget_lines.incurred_amount END
         RETURNING (xmax = 0) AS inserted`,
        [r.fiscalYear, r.category, r.allocated, r.incurred ?? 0, r.incurred != null],
      );
      if (res.rows[0]?.inserted === true) imported += 1;
      else updated += 1;
    }
    await client.query("COMMIT");
    return { imported, updated, years: [...years].sort((a, b) => a - b) };
  } catch {
