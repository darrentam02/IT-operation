...
async function extractPdfText(buffer: Buffer, _name: string): Promise<string | null> {
  ...
  try {
    // pdf.js initializes browser canvas globals when pdf-parse is imported.
    // Load it only for an explicit ingestion request so headless production
    // startup does not require DOMMatrix or a native canvas implementation.
...
/**
 * Indexes a single extracted PDF document straight through to embeddings —
 * the Storage → pgvector path. Mirrors the per-file step of the fixed-SOP
 * ingest but returns StoredChunks for per-document persistence instead of
 * writing globally. Returns null when extraction/embedding fails.
 */
export async function indexPdfBuffer(
  ...
  const text = await extractPdfText(buffer, documentName);
  if (!text) return null;
  const docs = chunkText(text, documentName, seedOffset);
  if (!docs.length) return null;
  const vectors = await embedTexts(docs.map((d) => d.content));
  if (!vectors || vectors.length !== docs.length) return null;
  return docs.map((d, i) => ({ ...d, embedding: vectors[i] }));
}
