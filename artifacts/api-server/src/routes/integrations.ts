...
import { runStorageSync, type StorageSyncResult } from "../lib/rag-storage";
...
// Compliance-style single-shot search.
...
// Run the Storage → pgvector sweep on demand (also scheduled at boot).
router.post("/rag/storage/sync", async (_req, res) => {
  try {
    const result: StorageSyncResult = await runStorageSync();
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error instanceof Error ? error.message : "storage sync failed" });
  }
});
