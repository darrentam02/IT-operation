...
import {
  ...
  BudgetImportBodySchema,
  BudgetImportErrorSchema,
  BudgetImportSuccessSchema,
...
// POST /budget/import — allocation-only upsert of client-parsed budget rows.
// All-or-nothing: a single invalid row rejects the whole file (422, per-row
// errors). paid_amount is never written (payments trigger owns it).
router.post("/budget/import", async (req, res) => {
  const parsed = BudgetImportBodySchema.safeParse(req.body);
  if (!parsed.success) {
    const errors = parsed.error.issues.map((issue) => {
      const [, rowIndex, field] = issue.path;
      return {
        row: typeof rowIndex === "number" ? rowIndex + 1 : 1,
        field: typeof field === "string" ? field : String(issue.path[0] ?? "body"),
        message: issue.message,
      };
    });
    res.status(422).json(BudgetImportErrorSchema.parse({ errors }));
    return;
  }
  try {
    const result = await budgetBreaker.run(() => importBudgetRows(parsed.data.rows));
    if (!result) throw new Error("budget import unavailable");
    res.json(BudgetImportSuccessSchema.parse(result));
  } catch (error) {
    if (error instanceof CircuitOpenError) {
      res.status(503).json({ error: "Budget service temporarily unavailable", code: "CIRCUIT_OPEN" });
      return;
    }
    res.status(500).json({ error: "budget import failed", code: "IMPORT_FAILED" });
  }
});
